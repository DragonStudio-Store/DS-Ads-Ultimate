...
public class Ads {
    ...
    public Ads(Main main, ConfigLoader configLoader, ConsoleManager console){
        Ads.main = main;
        Ads.configLoader = configLoader;
        Ads.console = console;
    }

    public void sendAd(ProxiedPlayer player, String adName, boolean bypassCooldown) {
        ...
        if (!configLoader.getAdServers(adName).contains(player.getServer().getInfo().getName())) {
            return;
        }
    ...
    public void sendChatMessage(ProxiedPlayer player, List<String> chatMessages, String adName) {
        if (configLoader.isChatAdEnabled(adName)) {
